...
<body>
    <div class="page-container">
        ...
        <div class="page-header">
            <div class="header-content">
                <div>
                    <h1 class="page-title">Released Documents</h1>
                    <p class="page-subtitle">Documents that have been released</p>
                </div>
        ...
        <div class="search-section">
            <form method="GET" class="search-form">
                ...
                <?php if (!empty($_GET['filtertext'])): ?>
                    <a href="viewdelivered.php" class="btn btn-secondary">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="table-section">
            <div class="table-header">
                <div class="table-title">Released Documents</div>
            ...
            <div class="table-container">
                ...
                if ($user_role === 'admin') {
                    ...
                    $month_sql = "SELECT COUNT(*) AS count FROM ".$datatable." WHERE document_status='$delivered' AND MONTH(date_added) = MONTH(CURRENT_DATE) AND YEAR(date_added) = YEAR(CURRENT_DATE)";
                } else {
                    $total_sql = "SELECT COUNT(*) AS count FROM ".$datatable." WHERE document_status='$delivered' AND added_by = $user_id";
                    $user_sql = $total_sql;
                    $month_sql = "SELECT COUNT(*) AS count FROM ".$datatable." WHERE document_status='$delivered' AND added_by = $user_id AND MONTH(date_added) = MONTH(CURRENT_DATE) AND YEAR(date_added) = YEAR(CURRENT_DATE)";
                }
                                    ...
                                    <td>
                                        <form action='details.php' method='POST' style="display: inline;">
                                            <button type="submit" name='RefID' value="<?php echo $row["serial_code"]; ?>" 
                                                    class="btn-action primary" title="Track Document">
                                                📍 Track
                                            </button>
                                        </form>
                                    </td>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <div class="empty-state-title">No released documents found</div>
                        <div class="empty-state-text">
                            <?php echo !empty($filtertext) ? 'No documents match your search criteria.' : 'No documents have been released yet.'; ?>
                        </div>
                        <?php if (!empty($filtertext)): ?>
                            <a href="viewdelivered.php" class="btn btn-primary">Clear Search</a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if ($total_pages > 1): ?>
                <div class="pagination">
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        ...
                        echo "<a href='viewdelivered.php?page=$i$current_filter' class='page-link $active_class'>$i</a> ";
                    }
                    ?>
                </div>
            <?php endif; ?>
    ...
    <script>
        ...
        document.getElementById('monthCount').textContent = '<?php echo $month_count; ?>';
